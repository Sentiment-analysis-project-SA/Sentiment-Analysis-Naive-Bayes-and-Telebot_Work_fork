...
class TrainingProcess:
    ...
    def read_files():
        ...
        inx_file = 0
        fX_open = open(path + "/" + files[inx_file] + "_text.txt",
                       mode="r", encoding=encoding)
        fy_open = open(path + "/" + files[inx_file] + "_labels.txt",
                       mode="r", encoding=encoding)
        inx_file = 0
        results = []
        for i in range(len(files)):
            with (open(path + "/" + files[i] + "_text.txt",
                      mode="r", encoding=encoding) as fx,
                  open(path + "/" + files[i] + "_labels.txt",
    ...
    def load():
        texts = []
        y = []
        loader = TrainingProcess.read_files()
    ...
    @time_count
    def model_training(X_train, X_test, y_train, y_test, model, words):
...
texts, y = TrainingProcess.load()
label_encoding = False
if label_encoding:
    X, words = TrainingProcess.my_label_encoding(texts, y)
else:
    X, words = TrainingProcess.my_vectorizer(texts, y)
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.33,
                                                    random_state  = 76)
TrainingProcess.model_training(X_train, X_test, y_train, y_test, model, words)
